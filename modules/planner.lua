...
function RoRota:ShouldRefreshFinisher(finisher, state, cache)
	...
	if finisher == "Rupture" and cfg and cfg.tasteForBlood then
		-- If target is immune to bleed but we don't have the buff, allow Rupture to get buff
		if self:IsTargetImmune(finisher) then
			if self:HasPlayerBuff("Taste for Blood") then
				-- Already have buff, skip Rupture on immune target
				return false
			else
				-- Don't have buff, allow Rupture to get it (skip other checks)
				if state.cp < (cfg.minCP or 1) then return false end
				if state.cp > (cfg.maxCP or 5) then return false end
				if state.energy < (cache.energyCosts[finisher] or 35) then return false end
				return true
			end
		end
	...
	if not cfg or not cfg.enabled then
		if RoRota.Debug and RoRota.Debug.enabled and finisher == "Envenom" then
			RoRota.Debug:Log("[FINISHER DEBUG] Envenom: not enabled or no config")
		end
		return false
	end
	...
	if finisher == "Eviscerate" then
		...
		if state.cp < (cfg.minCP or 1) then
			if RoRota.Debug and RoRota.Debug.enabled and finisher == "Envenom" then
				RoRota.Debug:Log(string.format("[FINISHER DEBUG] Envenom: CP %d < minCP %d", state.cp, cfg.minCP or 1))
			end
			return false
		end
		if state.cp > (cfg.maxCP or 5) then
			if RoRota.Debug and RoRota.Debug.enabled and finisher == "Envenom" then
				RoRota.Debug:Log(string.format("[FINISHER DEBUG] Envenom: CP %d > maxCP %d", state.cp, cfg.maxCP or 5))
			end
			return false
		end
	end
...
function RoRota:GetOptimalFinisher(state, cache)
	for i, finisher in ipairs(cache.finisherPrio) do
		...
		if RoRota.Debug and RoRota.Debug.enabled then
			RoRota.Debug:Log(string.format("[FINISHER] %s shouldUse=%s", finisher, tostring(shouldUse)))
		end
			if shouldUse then
				if finisher == "Cold Blood Eviscerate" then
...
function RoRota:GetOptimalFinisherWithTimeline(state, pacing, cache)
	...
	if pacing and pacing.shouldDump and state.cp >= 5 then
...
function RoRota:PlanRotation(state)
	...
	if state.cp == 5 then
		...
		if pacing and pacing.deadline and not pacing.deadline.virtual and pacing.deadline.expiresIn and pacing.deadline.expiresIn > 0 and pacing.deadline.expiresIn <= 5 then
		...
		local finisher, reason, data = self:GetOptimalFinisherWithTimeline(state, pacing, cache)
		if RoRota.Debug and RoRota.Debug.enabled then
			RoRota.Debug:Log(string.format("[PLAN] Finisher result: %s", tostring(finisher)))
		end
		if finisher then
			if RoRota.Debug and RoRota.Debug.enabled then
				RoRota.Debug:Log(string.format("[PLAN] Returning finisher: %s", finisher))
			end
			return finisher, reason, data
		end
		if RoRota.Debug and RoRota.Debug.enabled then
			RoRota.Debug:Log("[PLAN] No finisher, continuing to build phase")
		end
	end
	...
	if state.cp < 5 then
		...
		if cache.defensive.useRiposte then
			if self:IsReactiveUsable("Riposte") and self:HasEnoughEnergy("Riposte") then
				...
				if cache.targetHPPct >= targetMinHP and cache.targetHPPct <= targetMaxHP then
					if self.Conditions then
						local condPass = self.Conditions:CheckAbilityConditions("Riposte", {conditions = cache.defensive.riposteConditions or ""})
		...
		if cache.defensive.useSurpriseAttack then
			if self:IsReactiveUsable("Surprise Attack") and self:HasEnoughEnergy("Surprise Attack") then
				...
				if cache.targetHPPct >= targetMinHP and cache.targetHPPct <= targetMaxHP then
					if not self.Conditions or self.Conditions:CheckAbilityConditions("Surprise Attack", {conditions = cache.defensive.surpriseConditions or ""}) then
				...
				if cache.targetHPPct >= targetMinHP and cache.targetHPPct <= targetMaxHP then
					if cache.abilities.Hemorrhage.onlyElites and not self:IsTargetElite() then
						...
						if cache.abilities.Hemorrhage.onlyWhenMissing then
							local hasDebuff = self:HasTargetDebuff("Hemorrhage")
							shouldUse = not hasDebuff
							if self.Debug and self.Debug.enabled then
								self.Debug:Log("Hemorrhage check: hasDebuff="..tostring(hasDebuff)..", shouldUse="..tostring(shouldUse))
							end
						end
			...
			if state.energy >= cache.energyCosts["Ghostly Strike"] then
				if cache.targetHPPct <= (cache.defensive.ghostlyTargetMaxHP or 100) and 
				   ...
				   cache.playerHPPct <= (cache.defensive.ghostlyPlayerMaxHP or 100) then
					-- Check if targeted only setting is enabled
					local targetedCheck = true
					if cache.defensive.ghostlyTargetedOnly then
						if not UnitExists("target") or UnitName("target") ~= UnitName("targettarget") then
							targetedCheck = false
						end
					end
					if targetedCheck and (not self.Conditions or self.Conditions:CheckAbilityConditions("Ghostly Strike", {conditions = cache.defensive.ghostlyConditions or ""})) then
		...
		-- Skip Hemorrhage if onlyWhenMissing and debuff exists
		if builder == "Hemorrhage" and cache.abilities.Hemorrhage and cache.abilities.Hemorrhage.onlyWhenMissing and self:HasTargetDebuff("Hemorrhage") then
			if cache.db.secondaryBuilder and cache.db.secondaryBuilder ~= "Hemorrhage" then
				builder = cache.db.secondaryBuilder
			else
		...
		-- Check conditions on builder
		if builder == "Hemorrhage" and cache.abilities.Hemorrhage then
			if self.Conditions then
				local condPass = self.Conditions:CheckAbilityConditions(builder, cache.abilities.Hemorrhage)
				if not condPass then
					if cache.db.secondaryBuilder and cache.db.secondaryBuilder ~= "Hemorrhage" then
						builder = cache.db.secondaryBuilder
					else
						return nil, REASON.BUILD, 0
		...
		if RoRota.Debug and RoRota.Debug.enabled then
			RoRota.Debug:Log(string.format("[PLAN] Builder=%s Cost=%d Energy=%d", builder, builderCost, state.energy))
		end
		if state.energy >= builderCost then
			if RoRota.Debug and RoRota.Debug.enabled then
				RoRota.Debug:Log("[PLAN] Have energy, returning builder")
			end
		...
		if RoRota.Debug and RoRota.Debug.enabled then
			RoRota.Debug:Log("[PLAN] Not enough energy, returning TICK_WAIT")
		end
		return nil, REASON.TICK_WAIT, 0
	end
